...
Please note that if the application was downloaded by IDEasy it is automatically verifying the signature hash for you and will fail if the hash does not match the expected value.
...
The easiest way is to run link:setup.adoc[setup] and let it fail.
Then do the above workaround for `ideasy` and run the `setup` again.
Finally, you need to accept the terms and conditions and allow it to modify the disk to finish the setup.
