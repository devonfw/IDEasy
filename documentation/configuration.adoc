...
User-specific workspace configurations can be defined in `~/.ide/«IDE»/workspace/`.
This allows developers to configure personal IDE preferences that are applied across all projects and workspaces.
A good example is to copy your IntelliJ license to `~/.ide/intellij/workspace/setup/.intellij/config/idea.key` so it can be preserved on updates and across projects and workspaces.
