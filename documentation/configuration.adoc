...
User-specific workspace configurations can be defined in `~/.ide/settings/<<ide>>/workspace/`.
This allows developers to configure personal IDE preferences that are applied across all projects and workspaces.

IMPORTANT: The directory structure must include the `settings` subfolder (e.g. `~/.ide/settings/...`), otherwise the configuration will not be considered.
