...
```
+
The current command can be found https://github.com/dockur/macos?tab=readme-ov-file#via-docker-cli[here].
+
To avoid the container being deleted after stopping it, simply skip using the `--rm` flag.
Using the `--rm` flag doesn't affect the macOS session, i.e. when running the previous command (`docker run ...`) again, the macOS session will be restored.
