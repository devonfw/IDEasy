...
On MacOS you need to allow setup and IDEasy to be executed. This can be done in the MacOS settings under Privacy & Security in the section Security. The easiest way is to run setup, it will fail, then allow it and run it again. It will fail again, so you nead to allow IDEasy and run setup a last time. Last you need to accept the terms and conditions and allow it to modify the disk to finish the setup.
