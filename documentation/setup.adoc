...
* The `ide` alias is added to your shell config (`~/.bashrc` and `~/.zshrc`, search for: `alias ide="source /c/projects/_ide/bin/ide"`).
* The `completion` is added to your shell config (`~/.bashrc` and `~/.zshrc`, search for: `source /c/projects/_ide/completion`).
* The `IDE_ROOT` environment variable pointing to `c/projects` is added to your environment variables.
* The `/c/projects/_ide/bin` folder is added to your `PATH` environment variable.
