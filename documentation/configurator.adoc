...
* Additionally the expression `$[ask:«variable-name»]` can be used for values that cannot be preconfigured in the settings (e.g. a project specific service URL).
If `«variable-name»` is already defined, its value is used just like for a plain variable.
Otherwise the user is interactively asked to enter the value, which is then stored in his local `conf/ide.properties` so he will not be asked again.
For secret values like API keys or tokens use `$[secret:«variable-name»]` so the input is not echoed to the console while typing.
Please note that the entered value is currently stored as plain text in `conf/ide.properties` (which is not committed to git).
In batch mode such expressions are not resolved and a warning is logged instead.
