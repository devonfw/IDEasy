...
Each function is a small class implementing `ExpressionFunction` and registered by name in `ExpressionFunctionManager`.
For the up-to-date list of available functions and their arguments, defaults and examples, see the JavaDoc of the
link:https://github.com/devonfw/IDEasy/tree/main/cli/src/main/java/com/devonfw/tools/ide/expression/function[function package] instead of a second copy of
that information here, so the documentation cannot drift out of sync with the code:

* `@path` normalizes a path (e.g. to convert `$[IDE_HOME]/software/node/node.exe` to a form usable on the current operating system).
The optional 2nd argument is `unix` (the default, slashes) or `native` (backslashes on Windows).
* `@ask-variable` and `@ask-secret` ask the user for the value of a variable (in plain text, respectively with masked input) unless it is already defined.
The optional further arguments are the question, the configuration location to persist to (`settings`, `workspace`, `conf` or `home`, default `conf`, `@ask-variable` only) and a default value (`null` for none).
* `@if-windows`, `@if-mac`, `@if-linux` and `@if-unix` insert their single argument only if the current operating system matches and otherwise resolve to the
empty string, e.g. to select an OS specific value inline: `$[IDE_HOME]/software/mvn/bin/mvn@if-windows('.bat')@if-unix('.sh')`.
...
* Masking of `@ask-secret` values relies purely on the naming convention that a secret variable name ends with `SECRET`, `PASSWORD`, or `API_KEY`
(case-insensitive), so `@ask-secret` rejects a variable name that does not follow this convention.
This way a secret is reliably masked even when its value is read directly (e.g. because it was already defined in `ide.properties`) without going through
`@ask-secret` again.
