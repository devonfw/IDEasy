...
BTW: Windows has already an SSH client included.
...
For perfect integration, IDEasy automatically integrates Git Bash into Windows-Terminal during setup and makes it the default shell.
If you maintain a manual setup without IDEasy, you can still follow this https://www.timschaeps.be/post/adding-git-bash-to-windows-terminal/[manual integration].

.Integration of git-bash into Windows-Terminal
image::images/git-installer-windows-terminal-profile.png["Integrate git-bash into Windows-Terminal"]
...
IDEasy therefore replaces this with a title that tells you which project and workspace the shell is initialized for:

* `ideasy>«project»>«workspace»` when you are inside a workspace (e.g. `ideasy>devonfw>main`)
...
The workspace is derived from your current directory, so the title also follows a plain `cd` between workspaces.
...
They include tools like a file renamer, a way to order your windows on the screen, a color picker and more.
...
If you like a GUI to easily configure the scripts, that comes with a lot of extensions preinstalled, you should have a look at https://activaid.telgkamp.de/[Ac'tiveAid].
...
Convert them to https://en.wikipedia.org/wiki/Trivial_Graph_Format[Trivial Graph Format] `(.tgf)`, and run https://www.yworks.com/products/yed[yEd] to get an interactive visualization of your graph.
