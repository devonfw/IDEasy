...
We value homogeneity: when in doubt, follow what is already there rather than introducing a
different way of doing the same thing. Concretely:

* Follow the existing layout. Place a build, script or artifact next to what it belongs to (locality),
not in a detached central location. E.g. the macOS PKG build script lives at
`macos-installer/build-pkg.sh` and the Windows MSI build script at `windows-installer/build-msi.sh`
— each next to the installer it builds, not under `.github/workflows/`. (#2344)
* A later change follows the established pattern. If the repo already does something one way, a
later PR should follow that way instead of adding a second one.
* Prefer reusing existing infrastructure over reinventing the wheel — if Maven (or another build
tool) can do the job, use it.
* Mirror the counterpart for symmetry. When a peer module has something (a `README.adoc`, a
`pom.xml`), a newly added peer should carry the same so the layout stays homogeneous.
...
We use `this.` qualifier for accessing fields.
This is helpful to understand that a variable is actaully a field (`variable` may also be a local variable or method parameter, while `this.variable` can only be a field).
Especially with link:pull-request.adoc[Pull-Requests] where we do code-reviews also in the web browser this is helpful.
Ideally your IDE is auto-configured to prepend fields automatically with `this.` when formatting (or saving) your code.
However, methods should not be qualified with this.
That is pointless since a method can always be identified by its paranthesis (`method()` cannot be a variable and qualifying as `this.method()` is therefore considered pointless).
