...
If your CI build fails during PR / build (pull_request) with an error like:

    IllegalState End of answers reached!

especially for a specific test, this often happens due to missing executable permissions on certain scripts or resource files.
...
Commit and push the change, and the CI build should proceed successfully.
