...
* install the GUI and its dependencies into a self-contained Maven repository inside the local-dev installation
...
Because the GUI is resolved from that self-contained repository, `ide gui` works the same in every shell (IDEasy shell, Git Bash, cmd, ...) and does not depend on the context-dependent Maven local repository (`M2_REPO`), which could otherwise resolve a remote snapshot instead of your locally built GUI.
