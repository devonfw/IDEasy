...
To allow GitHub actions writing to our project (e.g. pushing changes) and reading organization team membership (external issue observer) we need a fine-grained access token.
...
. In `Organization permissions` select `Read` for `Members` and `Read` for `Projects`.
These are required by `.github/workflows/issue-pr-observer.yml` to resolve GitHub teams linked to the IDEasy board and their members.
...
. Copy your new secret token value into the text area and click on `Save changes`.
+
image::../images/AccessToken9.png["ACTION_PUSH_TOKEN"]
. Observe our GitHub Actions carefully the next day and check if all is working fine.
