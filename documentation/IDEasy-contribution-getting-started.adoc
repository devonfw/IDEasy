...
*Important*: This installation provides the latest published version of IDEasy, allowing you to execute commands like eclipse or docker, whose scripts are located inside `${IDE_HOME}$/scripts/command`.
However, for committing code, we use a different folder structure.
