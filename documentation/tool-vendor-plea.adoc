...
=== Proper release archives

If you download a portable release as `.tar.gz` or `zip` file and unpack it, also file permissions have to be properly.
However, we have seen things like this:
```
Cannot run program "~/projects/IDEasy/software/extra/graalvm/bin/native-image-configure": error=13, Permission denied
```
This is a buggy release then and not the fault of IDEasy that just unpacks the archive.
Tool vendors have to properly set executable flags in `tar` (or `zip`) files to make it work.
...
++++
<s>The worst is https://en.wikipedia.org/wiki/Windows_Subsystem_for_Linux[wsl] that does not offer any option to get the version or reliably figure out if WSL1 or WSL2 is installed.</s>
++++
Update: With newer versions of WSL this has been fixed. Thanks Microsoft!
```
$ wsl --version
WSL version: 2.6.1.0
