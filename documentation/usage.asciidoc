...
Note: IDEasy supports autocompletion (since `2021.04.001`). Currently this only works in bash (on windows use git bash). Simply type `ide` and hit `[Tab]` to get completion.
