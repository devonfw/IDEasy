...
|`«TOOL»_EXTRA_PLUGINS`|e.g. `copilot,docker`|Additional link:plugin.adoc[plugins] to install for the IDE `«TOOL»` (e.g. `VSCODE_EXTRA_PLUGINS`, `INTELLIJ_EXTRA_PLUGINS` or `ECLIPSE_EXTRA_PLUGINS`). Each entry refers to the filename of the plugin properties file without the `.properties` extension (e.g. `copilot` for `copilot.properties`) and not to the `id` configured inside that file. This option should only be set for individual users in `$IDE_HOME/conf/ide.properties` or `~/.ide/ide.properties` (and not in shared `settings`).
...
|`VSCODE_PROFILE_ENABLED`|e.g. `true`|Feature toggle (disabled by default) to isolate VSCode via a named `--profile` (`ideasy-«project»-«workspace»`) instead of `--user-data-dir`. This keeps the VSCode IPC lock at its default location so that OAuth callbacks via the `vscode://` protocol handler (e.g. GitHub/Copilot login) reach the already running window instead of opening a second one - see issue https://github.com/devonfw/IDEasy/issues/2058[#2058]. Note that enabling this starts with a fresh VSCode profile: settings, extension state and logins from the previous `--user-data-dir` are not migrated.
|=======================
