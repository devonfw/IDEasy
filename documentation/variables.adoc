...
|`USER`|e.g. `jdoe`|The login name of the current user. Provided as environment variable by Linux and macOS while Windows provides `USERNAME` instead, which is used as fallback so that `$[USER]` works on all platforms.
