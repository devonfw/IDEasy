...
See also: an link:https://github.com/devonfw/IDEasy/blob/main/cli/src/main/java/com/devonfw/tools/ide/variable/IdeVariables.java[Interface] that defines all the available variables. 
...
|`IDE_OPTIONS`|e.g. `http_proxy=http://proxy.host.com:8888
https_proxy=https://proxy.host.com:8443
no_proxy=.domain.com,localhost`|General options that will be applied to each call of `IDEasy`. Should typically be used for JVM options like link:proxy-support.adoc[proxy-support].
...
|`«TOOL»_EDITION`| |The edition of the tool `«TOOL»` to install and use (e.g. `ECLIPSE_EDITION`, `INTELLIJ_EDITION` or `DOCKER_EDITION`). Default of `DOCKER_EDITION` is `rancher`.
|*`«TOOL»_HOME`*|`$IDE_HOME/software/«tool»`|Path to installation of «tool» (e.g. MVN_HOME for maven)
|`«TOOL»_BUILD_OPTS`|e.g. `clean install`|The arguments provided to the build-tool `«TOOL»` in order to run a build. E.g.`clean install`
...
|`IDE_VARIABLE_SYNTAX_LEGACY_SUPPORT_ENABLED`|e.g. `true`|Enable/disable legacy support for devonfw-ide link:configurator.adoc[configuration templates] in IDE workspace folders.
...
|`HTTP_VERSIONS`|e.g. `HTTP 3, HTTP 2, HTTP 1.1`| The optional list of HTTP versions to try in the given order (e.g. "HTTP_2, HTTP_1_1"). This can be used as a workaround for network/VPN related issues - see issue https://github.com/devonfw/IDEasy/issues/1393[#1393].
|`JASYPT_OPTS`|e.g. `PBEWITHHMACSHA512ANDAES_256`|Options of jasypt.
