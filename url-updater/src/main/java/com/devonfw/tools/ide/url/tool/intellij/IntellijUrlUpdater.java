...
public class IntellijUrlUpdater extends IdeaBasedUrlUpdater {
  ...
  /**
   * This function moves releases later than 2025.2.6.1, which are unified releases of IntelliJ, but are still distributed internall as ultimate editions to the
   * community edition releases in the JSON objects, so that they are correctly recognized as community edition releases by the rest of the code.
   *
   * @param jsonObjects the array of JSON objects parsed from the response, which contains one object for the ultimate edition and one for the community
   *     edition. The function modifies this array in-place, so that after execution, all unified releases are moved to the community edition JSON object.
   */
  ...
  /**
   * This function determines, whether a given IntelliJ release is a unified release, meaning that there is no separate community edition available. This is the
   * case for all releases greater than 2025.2.6.1.
   *
