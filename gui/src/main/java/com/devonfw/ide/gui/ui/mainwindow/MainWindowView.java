...
public class MainWindowView extends BorderPane {
  ...
  /**
   * Adds the given tab to the tab pane and selects it. If a tab with the same identity is already present it is focused instead, so re-opening a tab does not
   * create a duplicate.
   *
   * <p>Identity is the tab's {@link Tab#getUserData() userData}, which tabs set to their stable NLS title key. De-duplicating by key (rather than by tab
   * instance) is what keeps a single tab open for a given key even though each open is handed a freshly created {@link Tab}.
   *
